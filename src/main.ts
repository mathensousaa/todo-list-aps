...
class InterfaceGrafica {
  ...
  constructor() {
    ...
    this.formularioEditarTarefa?.addEventListener("submit", (evento) => {
      ...
      const resposta = this.gerenciadorTarefa.editarTarefa(
        idTarefa,
        descricao,
        categoria,
        data
    ...
    this.formularioEditarCategoria?.addEventListener("submit", (evento) => {
      ...
      const resposta = this.gerenciadorCategoria.editarCategoria(
        idCategoria,
        nome
      );
  ...
  private fecharPopoverAoClicarFora(
    event: MouseEvent,
  ...
  private inicializarPopovers(): void {
    ...
    this.botoesFecharPopover = document.querySelectorAll(
  ...
  private adicionarEventosPopover(): void {
    if (this.popoversTarefa) {
      ...
      document.addEventListener("click", (evento: MouseEvent) => {
        this.botoesPopoverTarefa?.forEach((botao, indice) => {
          const popover = this.popoversTarefa?.[indice];
          this.fecharPopoverAoClicarFora(
            evento,
  ...
  private cardTarefa(tarefa: Tarefa): HTMLDivElement {
    ...
    const corCategoria = this.gerenciadorCategoria
      .obterCategorias()
      .find((categoria) => categoria.id === Number(tarefa.categoria))?.cor;
    const nomeCategoria = this.gerenciadorCategoria
      .obterCategorias()
      ...
      tarefa.status === "Concluída" ? "bg-green-500" : "bg-gray-100"
          ...
          <div class="flex-1">
            ...
            >
              ...
              <span class="text-xs ${
                this.verificarDataMenorQueAtual(tarefa.data)
                  ? "text-red-500"
                  : "text-gray-500"
              }">${this.converterDataParaTextoCorrido(tarefa.data)}</span>
            </div>
          </div>
  ...
  private atualizarOpcoesCategoria(
    campoSelecaoCategoria: HTMLSelectElement
  ): void {
