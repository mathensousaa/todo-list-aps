...
export class CategoriaController {
    ...
    criarCategoria(nome: string): { resultado: string, titulo: string, mensagem?: string } {
        try {
            const corDisponivel = this.corService.obterCorDisponivel();
            if (!corDisponivel) {
            ...
            this.corService.atualizarStatusCor(corDisponivel, true);
    ...
    editarCategoria(id: number, novoNome: string): { resultado: string, titulo: string, mensagem?: string } {
        try {
    ...
    excluirCategoria(id: number): { resultado: string, titulo: string, mensagem?: string } {
        try {
            const categoria = this.categoriaRepository.carregarCategoriaPorId(id);
            if (!categoria) {
                throw new Error(`Categoria ${id} não encontrada!`);
            ...
            this.corService.atualizarStatusCor(categoria.cor, false);
    ...
    private obterProximoId(): number {
        ...
        const ultimoId = todasCategorias.length > 0 ? todasCategorias[todasCategorias.length - 1].id : 0;
        return ultimoId + 1;
    }
}
